...
/**
 * PUT /devices/:deviceId/lastSeen
 *
 * Called by app's LastSeenReporter after every action.
 * Also called by HeartbeatWorker every 15 min.
 *
 * Body: {
 *   action: "sms_received" | "sms_sent" | "fcm_received" | "call_forwarded" |
 *           "heartbeat" | "app_open" | "boot",
 *   battery: 0-100 (optional, -1 if unknown),
 *   timestamp: epoch ms (optional, server uses Date.now() if missing)
 * }
 */
router.put("/:deviceId/lastSeen", async (req: Request, res: Response) => {
  try {
    ...
    // Also emit device:upsert so panel can update the full device card
    try {
      if (doc) {
        wsService.broadcastDeviceUpsert(doc);
      }
...
router.put("/:deviceId/fcm-token", async (req: Request, res: Response) => {
  try {
    ...
    // Also touch lastSeen — device is clearly alive if sending FCM token
    try {
...
router.put("/:deviceId/simSlots/:slot", async (req, res) => {
  try {
    ...
    // Touch lastSeen — device is alive (call forward result received)
    try {
...
router.post("/:id/sms", async (req: Request, res: Response) => {
  try {
    ...
    // Touch lastSeen — device pushed SMS data, it's alive
    try {
...
router.put("/:deviceId", async (req, res) => {
  try {
    ...
    // Extract fcmToken from metadata if present
