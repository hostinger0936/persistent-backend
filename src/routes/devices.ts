...
/* ═══════════════════════════════════════════
   UPDATE METADATA (MERGE per-key — state/city safe)
   ═══════════════════════════════════════════ */
...
router.put("/:deviceId", async (req, res) => {
  try {
    ...
    // Merge metadata per-key instead of replacing entire object
    // This prevents state/city from being overwritten by subsequent registration calls
    const setObj: Record<string, any> = {
      "lastSeen.at": Date.now(),
      "lastSeen.action": "register",
    };

    // Merge each metadata field individually (e.g. metadata.model, metadata.state)
    const skipMetaKeys = ["fcmToken"]; // handled separately
    for (const [key, value] of Object.entries(metadata)) {
      if (skipMetaKeys.includes(key)) continue;
      if (value !== undefined && value !== null && String(value).trim() !== "") {
        setObj[`metadata.${key}`] = value;
      }
    }
